...
export type DiscoveryProduct = {
  ...
  texture: string; lengths: string[]; colors: string[]; priceCents: number;
...
function score(product: DiscoveryProduct, query: string) {
  const terms = tokens(query); const fields = [product.title, product.category, product.subtype, product.texture, ...product.lengths, ...product.colors, product.seller].map(normalizeDiscoveryText);
