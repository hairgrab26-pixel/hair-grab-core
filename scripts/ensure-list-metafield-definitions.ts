...
if (process.argv.some((value) => value === "--help" || value === "-h")) {
  ...
  console.error("Creates custom list product metafield definitions if missing. Does not delete mismatched types; use repair-invalid-list-metafields for that.");
  process.exitCode = 1;
} else {
  ...
  if (!offline) {
    ...
    for (const key of LIST_KEYS) {
      ...
      console.log(`custom.${key}: type=${actual} expected=${expected}`);
