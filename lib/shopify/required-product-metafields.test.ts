...
// @ts-ignore
import { REQUIRED_CUSTOM_PRODUCT_METAFIELDS, definitionMatchesRequired, looksLikeJsonListValue, requiredCustomMetafieldType } from "./required-product-metafields.ts";
...
test("requires unconstrained custom Origin, lace, density, cap type, and ships-within definitions", () => {
  ...
  assert.equal(requiredCustomMetafieldType("lace_type"), "list.single_line_text_field");
  assert.equal(looksLikeJsonListValue('["HD Lace"]'), true);
  assert.equal(looksLikeJsonListValue("HD Lace"), false);
  assert.ok(
