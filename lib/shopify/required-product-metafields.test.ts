...
test("requires unconstrained custom Origin, lace, density, cap type, and ships-within definitions", () => {
  ...
  assert.equal(requiredCustomMetafieldType("cap_type", "single_line_text_field"), "list.single_line_text_field");
  assert.equal(requiredCustomMetafieldType("ships_within", "single_line_text_field"), "list.single_line_text_field");
  assert.ok(
