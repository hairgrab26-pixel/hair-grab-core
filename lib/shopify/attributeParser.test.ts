...
test("parses the 16 Admin metafields with tag fallbacks", () => {
  ...
  assert.deepEqual(parsed.density, ["180%"]);
  ...
  assert.deepEqual(parsed.shipsWithin, ["Same Day"]);
  assert.deepEqual(parsed.capType, ["Glueless"]);
...
test("reads Hair Type and City tags when metafields are missing", () => {
  ...
  assert.deepEqual(parsed.shipsWithin, ["48 Hours"]);
});
...
test("parses Origin plus Lace Size and Lace Type metafields and tags", () => {
  ...
  assert.deepEqual(fromMetafields.laceSize, ["13x4"]);
  ...
  assert.deepEqual(fromTags.laceSize, ["13x6"]);
